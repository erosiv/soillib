...
def main():
  ...
  wscale = np.array([20.0, 20.0, 0.1])  # World Scale [km] (x, y, z)
  ...
  nuplift = np.full((index[0], index[1]), 0.0, dtype=np.float32)
  for x in range(index[0]):
    for y in range(index[1]):
      if abs(x - index[0]/2) < index[0]/3 and abs(y - index[1]/2) < index[1]/3:
      #if x < index[0]/2:# and abs(y - index[1]/2) < 64:
        nuplift[x, y] = 1.0

  uplift = soil.buffer(soil.float32, index.elem(), soil.gpu)
  uplift[:] = 0.0

  nuplift = soil.buffer.from_numpy(nuplift)
  soil.add(uplift, nuplift.gpu())
  model.uplift = uplift
  ...
  param.uplift = 0.025  # [m/y]
  param.rainfall = 1.0
  param.evapRate = 0.0001
  
  param.gravity = 9.81
  param.viscosity = 0.000001
  param.bedShear = 0.00625

  param.critSlope = 0.57
  param.debrisCreepRate = 0.0025
  param.debrisSuspensionRate = 0.025
  param.debrisDepositionRate = 0.000025
  param.debrisBedShear = 0.0125
  param.debrisShear = 2E6 # Yield Stress [Pa]
  param.debrisDensity = 2500.0
  param.debrisViscosity = 0.0

  param.fluvialExponent = 0.01
  param.depositionRate = 0.00001
  param.suspensionRate = 0.0000008
  param.exitSlope = 0.025
  ...
  zip_save('/home/nickmcdonald/Datasets/erosion_multi_debris.zip', {
    "height": model.height,
