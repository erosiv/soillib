...
struct World {
    ...
    segpool(map.area*16)
  {
    ...
    for(auto [cell, pos]: map){
      cell.seg = segpool.get(sampler.get(glm::vec3(pos.x, pos.y, SEED%10000)/glm::vec3(512, 512, 1.0f)));
    }
    ...
    float max = 0.0f;
    for(auto [cell, pos]: map){
      if(cell.seg->height < min) min = cell.seg->height;
      if(cell.seg->height > max) max = cell.seg->height;
    }

    for(auto [cell, pos]: map){
      cell.seg->height = (cell.seg->height - min)/(max - min);
    }
  ...
  const inline float height(glm::ivec2 p){
    if(!map.oob(p))
      return World::config.scale*map.get(p)->seg->height;
    return 0.0f;
  }

  const inline void add(glm::ivec2 p, float h){
    if(!map.oob(p))
      map.get(p)->seg->height += h/World::config.scale;
  }
